...
// Common interface for rendering a graphical component to some given region of the window
...
pub struct AnimatedSprite {
    ...
    // The total time that the sprite has been alive, from which the current frame is derived
    current_time: f64,
}
...
impl AnimatedSprite {
    ...
    // If the value is negative, then we "rewind" the anime
    ...
    // Adds a certain amount of time, in seconds, to the 'current_time' of the animated sprite, so
    // that it knows when it must go to the next frame
    pub fn add_time(&mut self, dt: f64) {
        self.current_time += dt;

        // If we decide to go "back in time", this allows us to select the last frame whenever we
        // reach a negative one
